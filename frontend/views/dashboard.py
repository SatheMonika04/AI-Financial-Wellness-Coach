...
_SAMPLE_DATA = {
    ...
    "predicted_category": ["Food", "Transport", "Bills"],
    "amount": [1200.0, 850.0, 2000.0],
    "transaction": ["Debit", "Debit", "Debit"],
}
...
def _render_real_dashboard(df: pd.DataFrame) -> None:
    ...
    # Forecast remains based on monthly trend when enough history exists.
    if len(monthly_df) >= 2:
        forecast_values = []
        for i in range(1, 4):
